...
# Importamos los routers desde sus respectivos módulos para registrarlos.
...
async def initialize_application(db_conn: AsyncIOMotorDatabase):
    ...
    # Verifica y crea un superadministrador inicial si la base de datos está vacía de usuarios.
...
# Registro modular de routers. Cada 'include_router' añade un conjunto de endpoints.
...
api_router.include_router(customer_routes.router) # Suponiendo que ya lo creaste
api_router.include_router(purchase_order_routes.router) # Suponiendo que ya lo creaste
