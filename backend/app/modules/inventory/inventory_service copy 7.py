...
async def add_stock_from_goods_receipt(
    ...
    Por cada ítem en la recepción con cantidad positiva, crea un nuevo lote de
    inventario y desencadena la actualización del resumen de stock del producto.
    Esta función está diseñada para operar dentro de una transacción de base de datos.
    """
    ...
    # TODO: La lógica para asignar el almacén debe ser dinámica en el futuro,
    # posiblemente basada en el usuario o en datos de la propia recepción.
    warehouse_id_placeholder = ObjectId("60d5ec49e7e2d2001e4a0000")

    for item in receipt.items:
        # --- Punto de Depuración Clave ---
        # Si la cantidad recibida es cero, el stock no se modificará.
        if item.quantity_received <= 0:
            logger.warning(
                f"Omitiendo ítem SKU '{item.sku}' de la recepción '{receipt.receipt_number}'. "
                f"La cantidad recibida es cero o negativa ({item.quantity_received})."
            )
            continue

        logger.debug(
            f"Creando lote para producto ID '{item.product_id}' (SKU: {item.sku}) "
            f"con cantidad {item.quantity_received}."
        )
        ...
        new_lot = InventoryLotInDB(
            ...
            # El costo de adquisición se establece en 0. Se actualizará cuando se
            # procese la Factura de Compra, que es el documento financiero oficial.
            acquisition_cost=0,
...
async def create_initial_lot_for_product(
    ...
    if quantity <= 0:
        logger.info(f"No se creó lote inicial para el producto SKU '{product_sku}' porque la cantidad inicial es cero.")
        return
...
async def decrease_stock(
    ...
    """
    Disminuye el stock de un producto siguiendo una estrategia FIFO (Primero en Entrar, Primero en Salir).
    Esta es la función principal a ser llamada por otros servicios (ej: Módulo de Ventas).
    """
    ...
    if remaining_quantity_to_dispatch > 0:
        # Esta excepción indica una seria inconsistencia entre el stock resumido del producto y la suma de sus lotes.
        logger.error(
            f"INCONSISTENCIA DE STOCK para SKU '{product_document.get('sku')}'. "
            f"El stock total era {current_stock}, pero no se encontraron suficientes lotes para despachar {quantity_to_decrease} unidades."
        )
...
async def update_product_summary_from_lots(
    ...
    basándose en la suma agregada de sus lotes de inventario activos.
    Esta función es la única fuente de verdad para los totales de inventario de un producto.
    """
    ...
    # El pipeline de agregación calcula el stock y valor total directamente en la base de datos.
    pipeline = [
    ...
    # --- Punto de Depuración Clave ---
    # Verifica los valores calculados antes de actualizar el documento del producto.
    ...
    logger.info(f"PRODUCTO ID: {product_id_str} | STOCK CALCULADO DESDE LOTES: {total_stock}")
...
async def get_lots_by_product_id(database: AsyncIOMotorDatabase, product_id: str) -> List[InventoryLotOut]:
    ...
    la información con el SKU y nombre del producto para la visualización.
    """
    lot_repository = InventoryLotRepository(database)
    product_repository = ProductRepository(database)
