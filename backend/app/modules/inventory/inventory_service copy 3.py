...
async def add_stock_from_goods_receipt(
    ...
    Por cada ítem en la recepción, crea un nuevo lote de inventario.
    """
...
async def decrease_stock(
    ...
    if not product_document:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Producto con ID '{product_id}' no encontrado durante la disminución de stock.")
    ...
    # Obtener lotes disponibles ordenados por fecha de recepción (FIFO)
    available_lots = await lot_repo.find_available_by_product_id(
        product_id, 
        sort_options=[("received_on", ASCENDING)], 
        session=session
    )
    ...
    if remaining_quantity_to_dispatch > 0:
        # Esta excepción indica una seria inconsistencia entre el stock resumido y la suma de los lotes.
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=f"Inconsistencia de stock para SKU '{product_document.get('sku')}'. No se pudo satisfacer la cantidad solicitada desde los lotes."
        )
...
async def update_product_summary_from_lots(
    ...
    try:
        ...
        logger.error(f"ID de producto inválido '{product_id}' al actualizar resumen.")
        return
