...
import json # <-- Importación añadida para depuración
...
async def create_product(database: AsyncIOMotorDatabase, product_data: ProductCreate) -> ProductOutDetail:
    """
    ...
    if await product_repo.find_by_sku(product_data.sku):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"El SKU '{product_data.sku}' ya está registrado.")
    
    product_data_for_db = product_data.model_copy(update={"stock_quantity": 0, "average_cost": 0.0, "total_value": 0.0})
    ...
    if not created_product_doc:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error crítico: No se pudo recuperar el producto después de su creación.")

    return ProductOutDetail.model_validate(created_product_doc)

async def get_product_by_id(database: AsyncIOMotorDatabase, product_id: str) -> ProductOutDetail:
...
async def get_product_by_sku(database: AsyncIOMotorDatabase, sku: str) -> ProductOutDetail:
...
async def update_product_by_sku(database: AsyncIOMotorDatabase, sku: str, product_update_data: ProductUpdate) -> ProductOutDetail:
    """
...
async def get_products_paginated(
    ...
    # Se convierte a un formato serializable para la impresión
