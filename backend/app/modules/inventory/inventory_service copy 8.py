...
# Modelos
from app.modules.purchasing.purchasing_models import GoodsReceiptInDB, PurchaseOrderInDB
...
# Repositorio de otro módulo para obtener datos necesarios
...
async def add_stock_from_goods_receipt(
    ...
    Esta función ahora obtiene la Orden de Compra asociada para determinar el costo de
    adquisición de cada ítem en el momento de la recepción, valorizando el inventario
    correctamente desde el ingreso físico.
    """
    ...
    po_repository = PurchaseOrderRepository(database)

    # --- CORRECCIÓN CRÍTICA: Obtener la Orden de Compra para acceder al costo unitario ---
    po_doc = await po_repository.find_by_id(str(receipt.purchase_order_id), session=session)
    if not po_doc:
        raise ValueError(f"No se pudo encontrar la Orden de Compra ID '{receipt.purchase_order_id}' asociada a la recepción.")
    
    # Crear un mapa de producto_id -> unit_cost para una búsqueda eficiente.
    product_cost_map = {str(item['product_id']): item['unit_cost'] for item in po_doc.get('items', [])}
    ...
    for item in receipt.items:
        ...
        # Se busca el costo unitario desde la Orden de Compra.
...
async def decrease_stock(
    ...
    # (El resto de esta función no necesita cambios, se mantiene igual)
