...
async def add_stock_from_goods_receipt(
    ...
    Asegura que el tipo de dato de `product_id` sea ObjectId para consistencia en la BD.
    """
    ...
    for item in receipt.items:
        ...
        # --- CORRECCIÓN CRÍTICA Y LOGS DE VERIFICACIÓN DE TIPO ---
        # Se verifica el tipo de dato del ID y se asegura que sea un ObjectId.
        logger.debug(f"Verificación de tipo para product_id: '{item.product_id}' es de tipo {type(item.product_id)}")
        
        # Conversión explícita y defensiva para garantizar que el tipo en la BD es correcto.
        try:
            product_id_as_object = ObjectId(str(item.product_id))
        except InvalidId:
            logger.error(f"Error Crítico: el product_id '{item.product_id}' en la recepción no es un ObjectId válido. Omitiendo la creación del lote.")
            continue

        new_lot = InventoryLotInDB(
            product_id=product_id_as_object, # Se usa la variable convertida
        ...
        logger.info(f"Lote '{new_lot.lot_number}' creado. Desencadenando actualización de resumen para el producto ID '{product_id_as_object}'.")
...
async def create_initial_lot_for_product(
    ...
    # --- CORRECCIÓN DE TIPO ---
    # Asegura que el product_id siempre sea un ObjectId.
    logger.debug(f"Verificación de tipo para product_id inicial: '{product_id}' es de tipo {type(product_id)}")
