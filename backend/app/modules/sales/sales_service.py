    ...
    async with await database.client.start_session() as session:
        async with session.start_transaction():
            ...
            if order_status not in [SalesOrderStatus.CONFIRMED, SalesOrderStatus.PARTIALLY_SHIPPED]:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"No se puede crear un despacho para una orden en estado '{order_status.value}'."
                )
            ...
            # --- CORRECCIÓN CRÍTICA ---
            # Se extrae el 'customer_id' de la orden de venta original y se añade al
            # objeto del despacho para cumplir con el requisito del modelo ShipmentInDB.
            customer_id_from_order = order_doc.get("customer_id")
            if not customer_id_from_order:
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="La orden de venta no tiene un cliente asociado.")
            ...
            # Lógica para actualizar el stock de inventario (aquí se asumiría que 'decrease_stock' existe)
            for item in shipment_to_db.items:
                await inventory_service.decrease_stock(
                    database,
                    product_id=str(item.product_id),
                    quantity_to_decrease=item.quantity_shipped, # Usa la cantidad despachada
                    session=session
                )
            
            # Lógica para actualizar estado de la OV a 'partially_shipped' o 'shipped' (omitida por brevedad)

    # El retorno se hace fuera de la transacción para asegurar que los datos sean visibles
    new_shipment_doc = await shipment_repo.find_by_id(str(inserted_id))
    if not new_shipment_doc:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error al recuperar el despacho recién creado.")
