...
class PurchaseOrderRepository:
    ...
    async def insert_one(self, order_doc: Dict[str, Any], session: Optional[AsyncIOMotorClientSession] = None) -> ObjectId:
        ...
        # El driver motor devuelve el ObjectId real insertado, que es lo que debemos retornar.
    ...
    async def find_by_id(self, order_id: str, session: Optional[AsyncIOMotorClientSession] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return await self.collection.find_one({"_id": object_id_to_find}, session=session)
        except InvalidId:
            # Si el string de entrada no es un ObjectId válido, es imposible que exista en la BD.
            return None
    ...
    async def update_one_by_id(self, order_id: str, update_data: Dict[str, Any], session: Optional[AsyncIOMotorClientSession] = None) -> int:
        ...
        Nota: El diccionario `update_data` debe contener los operadores de MongoDB (ej: {"$set": {...}}).
        """
        try:
