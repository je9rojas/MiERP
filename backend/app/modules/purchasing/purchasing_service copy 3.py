...
async def create_purchase_order(db: AsyncIOMotorDatabase, po_data: PurchaseOrderCreate, user: UserOut) -> PurchaseOrderOut:
    ...
    # --- CORRECCIÓN CRÍTICA ---
    # Convertimos el modelo a un diccionario para la BD.
    # Usamos `exclude={'id'}` para evitar que model_dump convierta el ObjectId a string.
    # Luego, añadimos manualmente el campo `_id` con el objeto ObjectId real.
    # Esto asegura que el tipo de dato en MongoDB sea `ObjectId` y no `string`.
...
async def _get_new_po_status_after_receipt(db: AsyncIOMotorDatabase, po_id: PyObjectId, session=None) -> PurchaseOrderStatus:
    ...
    if not po_doc_raw:
        # Se añade validación para evitar errores si la OC no se encuentra.
