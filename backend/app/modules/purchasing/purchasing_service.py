...
from .purchasing_models import (
    ...
    PurchaseBillInDB, PurchaseBillOut, PurchaseOrderItemCreate # <-- CORRECCIÓN APLICADA AQUÍ
)
...
async def _generate_sequential_number(repo, prefix: str) -> str:
    """Genera un número secuencial para un documento (ej: OC-2025-00001)."""
...
async def _get_new_po_status(db: AsyncIOMotorDatabase, po_id: str) -> PurchaseOrderStatus:
    """
    Calcula el nuevo estado de una OC basándose en las cantidades ordenadas
    versus las cantidades totales recibidas en todas sus facturas asociadas.
    """
...
async def process_purchase_receipt(db: AsyncIOMotorDatabase, po_id: str, bill_data: PurchaseBillCreate, user: UserOut) -> PurchaseBillOut:
    ...
    async with await client.start_session() as session:
        async with session.start_transaction():
            ...
            bill_to_db = PurchaseBillInDB(
                ...
                supplier_invoice_number=bill_data.supplier_invoice_number,
    ...
    try:
        ...
        update_po_data = {
            ...
            "related_bill_ids": po_doc_after.related_bill_ids + [inserted_id]
        }
...
async def get_bill_by_id(db: AsyncIOMotorDatabase, bill_id: str) -> PurchaseBillOut:
    """Obtiene una única Factura de Compra por su ID, enriqueciendo los datos del proveedor."""
    bill_repo = PurchaseBillRepository(db)
...
async def create_purchase_order_from_file(db: AsyncIOMotorDatabase, file: UploadFile, user: UserOut) -> PurchaseOrderOut:
    """Crea y recibe AUTOMÁTICAMENTE una Orden de Compra a partir de un archivo CSV de inventario inicial."""
    try:
    ...
    bill_data = PurchaseBillCreate(
        supplier_invoice_number=f"INV-INICIAL-{initial_po.order_number}",
        received_date=datetime.now(timezone.utc),
        notes="Recepción automática de la carga de inventario inicial.",
        items=bill_items
    )
