...
async def create_goods_receipt(database: AsyncIOMotorDatabase, receipt_data: GoodsReceiptCreate, current_user: UserOut) -> GoodsReceiptOut:
    """Crea una Recepción de Mercancía de forma transaccional."""
    po_repo = PurchaseOrderRepository(database)
    receipt_repo = GoodsReceiptRepository(database)
    inserted_id = None
    ...
    async with await database.client.start_session() as session:
        try:
            async with session.start_transaction():
                ...
                # Paso 1: Obtener y validar la Orden de Compra dentro de la transacción.
                ...
                # --- CORRECCIÓN CRÍTICA ---
                # Paso 3: Invocar al servicio de inventario pasándole el documento de la OC recuperado.
                # Esto asegura que el servicio de inventario opera dentro de la misma sesión y con
                # los datos correctos sin necesidad de hacer una nueva consulta.
                logger.info(f"Invocando al servicio de inventario para actualizar stock para la recepción ID: {inserted_id}")
                logger.debug(f"Pasando OC al servicio de inventario (ID: {po_doc['_id']}, items: {len(po_doc.get('items', []))})")
                
                await inventory_service.add_stock_from_goods_receipt(
                    database=database,
                    receipt=receipt_to_db,
                    purchase_order_doc=po_doc, # <-- Se pasa el documento completo
                    session=session
                )
