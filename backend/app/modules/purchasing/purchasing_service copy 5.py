...
async def create_goods_receipt(db: AsyncIOMotorDatabase, receipt_data: GoodsReceiptCreate, user: UserOut) -> GoodsReceiptOut:
    ...
    async with await client.start_session() as session:
        async with session.start_transaction():
            ...
            # --- CORRECCIÓN CRÍTICA ---
            # Se excluye `received_date` del model_dump porque se proporciona explícitamente
            # después, ya convertido a datetime, evitando el TypeError.
            receipt_to_db = GoodsReceiptInDB(
                **receipt_data.model_dump(exclude={'received_date'}),
            ...
            # Se pasa la instancia Pydantic, que es lo que el servicio de inventario espera
...
async def create_purchase_bill(db: AsyncIOMotorDatabase, bill_data: PurchaseBillCreate, user: UserOut) -> PurchaseBillOut:
    ...
    bill_to_db = PurchaseBillInDB(
        **bill_data.model_dump(exclude={'invoice_date', 'due_date'}),
