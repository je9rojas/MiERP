...
class SalesOrderPDFService:
    ...
    def __init__(self, order_data: Dict[str, Any], document_title: str, company_info: Dict[str, Any]):
        ...
        # (CORREGIDO) Se añade la definición de 'line_height' que faltaba.
    ...
    def _draw_billing_info(self):
        ...
        p.drawOn(self.c, self.margin, y_start - p.height - 5)
        ...
        p_customer.drawOn(self.c, x_customer_start, y_start - p_customer.height - 5)
    ...
    def _draw_items_table(self):
        ...
        for i, item in enumerate(items):
            if i % 2 != 0:
                self.c.setFillColor(self.color_light_grey)
                self.c.rect(self.margin, current_y, self.width - 2 * self.margin, -30, stroke=0, fill=1)
            ...
            self.c.drawRightString(x_positions[1] - 15, current_y - 8, str(quantity))
            self.c.drawRightString(x_positions[2] - 15, current_y - 8, f"{self.currency_symbol} {unit_price:.2f}")
            self.c.drawRightString(x_positions[3] - 15, current_y - 8, f"{self.currency_symbol} {subtotal:.2f}")
    ...
    def _draw_notes_and_totals(self):
        ...
        if notes:
            ...
            notes_text.drawOn(self.c, self.margin, y_start - notes_text.height - 5)

    def _draw_footer(self):
        """Draws a clean footer with contact information."""
        y_start = 1 * inch
        self.c.setStrokeColor(colors.lightgrey)
        self.c.line(self.margin, y_start + self.line_height + 5, self.width - self.margin, y_start + self.line_height + 5)
        ...
        self.c.drawCentredString(self.width / 2.0, y_start - self.line_height, "Gracias por su preferencia. Este documento no es un comprobante de pago.")