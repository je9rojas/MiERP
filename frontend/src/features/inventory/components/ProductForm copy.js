 ...
 *
 * Arquitectura:
 * - Formik: Gestiona el estado del formulario.
 * - Yup: Proporciona validación de datos a través de un esquema centralizado.
 * - Material-UI: Construye la interfaz de usuario.
 * - Hook Personalizado (`useProductForm`): Aísla la lógica de inicialización y formateo.
 * - Sub-componentes Memoizados: Descompone la UI para mejorar rendimiento y legibilidad.
 */

// SECCIÓN 1: IMPORTACIONES DE MÓDULOS
import React, { useMemo, useCallback } from 'react';
import { Formik, Form, FieldArray } from 'formik';
import { Box, TextField, Button, Grid, MenuItem, Typography, Divider, IconButton, Paper } from '@mui/material';
...
// SECCIÓN 3: SUB-COMPONENTES DE UI REUTILIZABLES
                ...
                {formikProps.values[name].map((item, index) => (
                    <Grid container spacing={2} key={index} sx={{ mb: 2, alignItems: 'center' }}>
                        <Grid item xs={10} sm={5}>
                            <TextField fullWidth label={fieldLabels.brand} name={`${name}.${index}.brand`} value={item.brand} onChange={formikProps.handleChange} />
                        </Grid>
                        <Grid item xs={10} sm={5}>
                            <TextField fullWidth label={fieldLabels.code} name={`${name}.${index}.code`} value={item.code} onChange={formikProps.handleChange} />
                        </Grid>
                        <Grid item xs={2}>
                            <IconButton disabled={formikProps.values[name].length <= 1} onClick={() => remove(index)} color="error">
...
const useProductForm = ({ initialData, isEditMode }) => {
    const initialValues = useMemo(() => {
        // Función para formatear valores que vienen de la API. '' para campos vacíos.
        const formatApiValue = (value) => (value === null || value === undefined ? '' : String(value));
        
        // --- CORRECCIÓN: Definir valores por defecto para el modo CREACIÓN ---
        const defaults = {
            sku: '', name: '', brand: '', category: '', product_type: 'n_a', shape: '',
            description: '', main_image_url: '',
            // --- CORRECCIÓN: Valores numéricos inician en 0 para nuevos productos ---
            price: '0',
            weight_g: '0',
            points_on_sale: '0',
            // Estos son solo para el lote inicial en modo creación
            stock_quantity: '0',
            average_cost: '0',
            dimensions: { ...INITIAL_DIMENSIONS_STATE },
            oem_codes: [INITIAL_REFERENCE_STATE],
            cross_references: [INITIAL_REFERENCE_STATE],
            applications: [INITIAL_APPLICATION_STATE],
        };
        
        // Si hay datos iniciales (modo edición), formatearlos. Si no, usar los defaults.
        const baseData = isEditMode && initialData ? {
            sku: formatApiValue(initialData.sku),
            name: formatApiValue(initialData.name),
            brand: formatApiValue(initialData.brand),
            category: formatApiValue(initialData.category),
            product_type: formatApiValue(initialData.product_type),
            shape: formatApiValue(initialData.shape),
            description: formatApiValue(initialData.description),
            main_image_url: formatApiValue(initialData.main_image_url),
...
const ProductForm = ({ onSubmit, initialData = {}, isSubmitting = false }) => {
    ...
    const { initialValues, validationSchema } = useProductForm({ initialData, isEditMode });
    
    // El callback de `onSubmit` se pasa directamente a Formik.
    ...
    return (
        <Formik
            ...
            enableReinitialize // Permite que el formulario se reinicie cuando `initialData` cambia.
        >
            {(formikProps) => (
                <Form noValidate>
                    <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 2, boxShadow: 'none' }}>
                        ...
                        <Grid container spacing={3} sx={{ mb: 3 }}>
                            <Grid item xs={12} sm={4}><TextField fullWidth required name="sku" label="SKU / Código" value={formikProps.values.sku} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur} error={formikProps.touched.sku && Boolean(formikProps.errors.sku)} helperText={formikProps.touched.sku && formikProps.errors.sku} disabled={isEditMode} /></Grid>
                            ...
                            <Grid item xs={12}>
                                <Grid container spacing={2}>
                                    <Grid item xs={12} sm={4}><TextField select required fullWidth label="Categoría" name="category" value={formikProps.values.category} onChange={(e) => { const isFilter = e.target.value === 'filter'; formikProps.setFieldValue('category', e.target.value); if (!isFilter) { formikProps.setFieldValue('product_type', 'n_a'); formikProps.setFieldValue('shape', 'n_a'); formikProps.setFieldValue('dimensions', INITIAL_DIMENSIONS_STATE); } }} onBlur={formikProps.handleBlur} error={formikProps.touched.category && Boolean(formikProps.errors.category)} helperText={formikProps.touched.category && formikProps.errors.category}>{PRODUCT_CATEGORIES.map(opt => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}</TextField></Grid>
                                    {formikProps.values.category === 'filter' && (
                                        <>
                                            <Grid item xs={12} sm={4}><TextField select required fullWidth label="Tipo de Filtro" name="product_type" value={formikProps.values.product_type} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur} error={formikProps.touched.product_type && Boolean(formikProps.errors.product_type)} helperText={formikProps.touched.product_type && formikProps.errors.product_type}>{FILTER_TYPES.map(opt => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}</TextField></Grid>
                                            <Grid item xs={12} sm={4}><TextField select fullWidth label="Forma" name="shape" value={formikProps.values.shape || ''} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur}>{PRODUCT_SHAPES.map(opt => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}</TextField></Grid>
                                        </>
                                    )}
                                </Grid>
                            </Grid>
                        ...
                        <Grid container spacing={3} sx={{ mb: 3 }}>
                            {/* --- CORRECCIÓN: La lógica `!isEditMode` fue removida para mostrar siempre los campos --- */}
                            <Grid item xs={12} sm={6} md={3}>
                                <TextField
                                    fullWidth type="number" label="Stock Total" name="stock_quantity"
                                    value={formikProps.values.stock_quantity}
                            ...
                            <Grid item xs={12} sm={6} md={3}>
                                <TextField
                                    fullWidth type="number" label="Costo Promedio" name="average_cost"
                                    value={formikProps.values.average_cost}
                                    disabled // El costo se gestiona por lotes, aquí es solo informativo
                                    helperText="Se calcula desde los lotes."
                                />
                            </Grid>
                            <Grid item xs={12} sm={6} md={3}>
                                <TextField fullWidth required type="number" label="Precio de Venta" name="price" value={formikProps.values.price} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur} error={formikProps.touched.price && Boolean(formikProps.errors.price)} helperText={formikProps.touched.price && formikProps.errors.price} />
                            </Grid>
                            <Grid item xs={12} sm={6} md={3}>
                                <TextField fullWidth type="number" label="Peso (g)" name="weight_g" value={formikProps.values.weight_g} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur} error={formikProps.touched.weight_g && Boolean(formikProps.errors.weight_g)} helperText={formikProps.touched.weight_g && formikProps.errors.weight_g} />
                            </Grid>
                            <Grid item xs={12} sm={6} md={3}>
                                <TextField fullWidth type="number" label="Puntos por Venta" name="points_on_sale" value={formikProps.values.points_on_sale} onChange={formikProps.handleChange} onBlur={formikProps.handleBlur} error={formikProps.touched.points_on_sale && Boolean(formikProps.errors.points_on_sale)} helperText={formikProps.touched.points_on_sale && formikProps.errors.points_on_sale} />
                            </Grid>
                        </Grid>
                        <Divider sx={{ mb: 3 }} />
