...
// COMPONENTE DE FORMULARIO COMPARTIDO PARA LA CREACIÓN Y EDICIÓN DE PRODUCTOS
...
// Este componente se encarga de renderizar dinámicamente los campos de especificaciones
// según la forma del producto seleccionada.
  ...
  return (
    ...
    >
      {({ values, errors, touched, handleChange, handleBlur, setFieldValue, isValid, dirty }) => (
        <Form noValidate>
          {/* ... (Secciones de Información Principal, Comercial y Especificaciones no cambian) ... */}
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={3}><TextField fullWidth type="number" label="Puntos" name="points_on_sale" value={values.points_on_sale} onChange={handleChange} /></Grid>
          </Grid>
          <Divider />
          ...
          <Divider />
          <FieldArray name="oem_codes">{({ push, remove }) => (<Box sx={{ mt: 3, mb: 3 }}><Typography variant="h6" gutterBottom>Códigos de Equipo Original (OEM)</Typography>{values.oem_codes.map((oem, index) => (<Grid container spacing={2} key={index} sx={{ mb: 2, alignItems: 'center' }}><Grid item xs={10} sm={5}><TextField fullWidth label="Marca Vehículo" name={`oem_codes.${index}.brand`} value={oem.brand} onChange={handleChange} /></Grid><Grid item xs={10} sm={5}><TextField fullWidth label="Código Original" name={`oem_codes.${index}.code`} value={oem.code} onChange={handleChange} /></Grid><Grid item xs={2}>{values.oem_codes.length > 1 && <IconButton onClick={() => remove(index)} color="error"><RemoveCircleOutlineIcon /></IconButton>}</Grid></Grid>))}<Button startIcon={<AddCircleOutlineIcon />} onClick={() => push({ brand: '', code: '' })}>Añadir Código OEM</Button></Box>)}</FieldArray>
          <Divider />
          <FieldArray name="cross_references">{({ push, remove }) => (<Box sx={{ mt: 3, mb: 3 }}><Typography variant="h6" gutterBottom>Referencias Cruzadas (Aftermarket)</Typography>{values.cross_references.map((ref, index) => (<Grid container spacing={2} key={index} sx={{ mb: 2, alignItems: 'center' }}><Grid item xs={10} sm={5}><TextField fullWidth label="Marca Referencia" name={`cross_references.${index}.brand`} value={ref.brand} onChange={handleChange} /></Grid><Grid item xs={10} sm={5}><TextField fullWidth label="Código Referencia" name={`cross_references.${index}.code`} value={ref.code} onChange={handleChange} /></Grid><Grid item xs={2}>{values.cross_references.length > 1 && <IconButton onClick={() => remove(index)} color="error"><RemoveCircleOutlineIcon /></IconButton>}</Grid></Grid>))}<Button startIcon={<AddCircleOutlineIcon />} onClick={() => push({ brand: '', code: '' })}>Añadir Referencia</Button></Box>)}</FieldArray>
