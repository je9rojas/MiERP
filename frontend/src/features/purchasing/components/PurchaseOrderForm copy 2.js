                            ...
                            <Grid container spacing={2} alignItems="center">
                                <Grid item xs={12} md={5}>
                                    <Autocomplete
                                        ...
                                        renderInput={(params) => (
                                            <TextField {...params} label="Producto" required error={touched.items?.[index]?.product && Boolean(errors.items?.[index]?.product)} InputProps={{ ...params.InputProps, endAdornment: (<>{isLoadingProducts ? <CircularProgress color="inherit" size={20} /> : null}{params.InputProps.endAdornment}</>),}}/>
                                        )}
                                    />
                                </Grid>
                                <Grid item xs={6} md={2}>
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={es}>
            ...
            >
                {({ values, errors, touched, setFieldValue }) => {
                    // --- CORRECCIÓN CLAVE ---
                    // Se calcula el total directamente. Llamar a useMemo aquí violaba las reglas de los Hooks.
