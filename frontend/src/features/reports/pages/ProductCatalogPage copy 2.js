 ...
 * @description Permite a los usuarios generar catálogos en PDF en tres modos:
 * general, filtrado por marca/tipo, o personalizado seleccionando productos específicos.
 */
...
const ProductCatalogPage = () => {
    ...
    const { mutate: generateCatalog, isPending: isGenerating } = useMutation({
        ...
        onSuccess: (pdfBlob, variables) => {
            ...
            } else if (variables.brands && variables.brands.length > 0) {
    ...
    const handleGenerateClick = useCallback(() => {
        ...
        switch (reportType) {
            ...
            case 'all':
            default:
                // No se añaden más filtros para el catálogo general.
                break;
        }
        ...
        <Container maxWidth="md" sx={{ my: 4 }}>
            <Paper sx={{ p: { xs: 2, md: 4 }, borderRadius: 2, boxShadow: 3 }}>
                ...
                <Grid container spacing={4} sx={{ mt: 2 }}>
                    ...
                    {reportType === 'by_brand' && (
                        <Grid item xs={12} container spacing={2}>
                            ...
                            <Grid item xs={12} sm={6}>
                                <TextField select fullWidth name="brands_selector" label="Seleccione una o más Marcas" variant="outlined" value={selectedBrands} onChange={(e) => setSelectedBrands(e.target.value)} SelectProps={{ multiple: true, renderValue: (selected) => selected.join(', ') }}>
                                    {AVAILABLE_BRANDS.map((option) => (<MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>))}
                                </TextField>
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <TextField select fullWidth name="product_types_selector" label="Tipo de Filtro (Opcional)" variant="outlined" value={selectedTypes} onChange={(e) => setSelectedTypes(e.target.value)} SelectProps={{ multiple: true, renderValue: (selected) => selected.join(', ') }}>
                    ...
                    {reportType === 'custom' && (
                        <Grid item xs={12}>
                             <Typography variant="h6" gutterBottom>Añada Productos Específicos al Catálogo</Typography>
