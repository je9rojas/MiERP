...
import { getCustomersAPI } from '../../crm/api/customersAPI'; // Asume la creación de este archivo
            ...
            >
                {({ values, errors, touched, setFieldValue }) => {
                    const totalAmount = useMemo(() =>
                        values.items.reduce((acc, item) => acc + (Number(item.quantity) * (item.product?.price || 0)), 0),
                        [values.items]
                    );
