                                ...
                                <Grid item xs={12} md={5}>
                                    <Autocomplete
                                        ...
                                        onChange={(_event, newValue) => {
                                            // Al seleccionar un producto, se actualizan ambos campos: el producto y el precio unitario.
...
const SalesOrderForm = ({
    ...
    const initialValues = useMemo(() => {
        const defaults = {
            ...
            order_date: new Date(),
            notes: '',
            items: [{ product: null, quantity: 1, unit_price: 0 }], // Se añade unit_price al item inicial
        };
        
        if (isEditMode) {
            // Lógica para poblar el formulario en modo edición (se mantiene)
            return { /* ... */ };
        }
