        ...
        return accessibleItems.map((item) => {
            ...
            return (
                <React.Fragment key={item.text}>
                    <ListItemButton onClick={() => handleCollapseClick(item.text)} sx={{ minHeight: 48, justifyContent: open ? 'initial' : 'center', px: 2.5, bgcolor: isParentActive && !openCollapse[item.text] ? 'action.hover' : 'transparent' }}>
                        <ListItemIcon sx={{ minWidth: 0, mr: open ? 3 : 'auto', justifyContent: 'center' }}>{item.icon}</ListItemIcon>
                        <ListItemText primary={item.text} sx={{ opacity: open ? 1 : 0 }} />
